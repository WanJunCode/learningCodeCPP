...
namespace pcpp
{
	/**
	 * @class TLVRecord
	 * 对于 Type-Length-Value 记录的封装类，并不创建和修改TLV记录，只提供处理函数。具有多个抽象函数，在派生类中进行实现
	...
	protected:
		...
		struct TLVRawData
		{
			...
			uint8_t recordLen;
			/** Record value (variable size) 柔性数组*/
			uint8_t recordValue[];
		};
	...
	public:
		 ...
		 * Notice this return value is a copy of the data, not a pointer to the actual data 返回值是一份拷贝
		...
		T getValueAs(size_t offset = 0) const
		{
			...
			T result;
			memcpy(&result, m_Data->recordValue + offset, sizeof(T));		// 直接复制内存
			return result;
		}
		 ...
		 * @param[in] valueOffset An optional parameter that specifies where to start setting the record data (default set to 0). 
		 * For example:
		 * 如果数据长度是 20 bytes， 你只需要去设置最后的 4 bytes
	 ...
	 * 从byte流中读取 TLV 记录，包含提取和计算TLV记录的函数
	 * A class for reading TLV records data out of a byte stream. This class contains helper methods for retrieving and
	 * counting TLV records. This is a template class(模板类) that expects template argument class derived from TLVRecord.
	 */
	template<typename TLVRecordType>
	class TLVRecordReader
	{
	private:
		mutable size_t m_RecordCount;		// 记录数量； 在 const 函数中可以修改
	...
	public:
		 ...
		 * return true
		 * 返回值需要使用 TLVRecordType.isNull() 判断是否为空
		 */
		TLVRecordType getFirstTLVRecord(uint8_t* tlvDataBasePtr, size_t tlvDataLen) const
		{
			...
			if (tlvDataLen == 0 || resRec.getTotalSize() == 0)			// getTotalSize 虚函数
				resRec.assign(NULL);
		 ...
		 * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream		指针指向TLV原始数据流
		 * @param[in] tlvDataLen The TLV data byte stream length
		 ...
		 * @param[in] recordType The record type to search for					查找的记录类型
		 * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream
		 * @param[in] tlvDataLen The TLV data byte stream length
		...
		/**
		 * 获得 TLV 记录的数量，（为了更高的效率，只有第一次执行会计算，后面的调用都返回cached）
	...
	protected:
		...
		uint8_t* m_RecValue;			// 值		V
		uint8_t m_RecValueLen;			// 长度		L
		uint8_t m_RecType;				// 类型		T
	};
}
