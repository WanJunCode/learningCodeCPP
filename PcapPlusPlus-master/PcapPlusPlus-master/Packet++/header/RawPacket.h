...
#define PCPP_MAX_PACKET_SIZE 65536
	...
	/**
	 * @class RawPacket
	 * This class holds the packet as raw (not parsed) data (未处理数据). The data is held as byte array. In addition to the data itself
	...
	{
	protected:
		uint8_t* m_RawData;									// 未处理数据指针
		int m_RawDataLen;									// 未处理数据长度
		int m_FrameLength;									// 帧数据长度
		timeval m_TimeStamp;								// 时间戳，从网卡接受数据的时间
		bool m_DeleteRawDataAtDestructor;					// 在析构函数是否删除 raw data
		bool m_RawPacketSet;								// 是否设置了 raw data
		LinkLayerType m_LinkLayerType;						// 链路层协议类型
		void init();
