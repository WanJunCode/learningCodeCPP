...
private:
	...
	struct TcpReassemblyData
	{
		int numOfSides;
		int prevSide;
		TcpOneSideData twoSides[2];				// 存储连接两端的数据
		ConnectionData connData;				// 存储连接信息
	...
	OnTcpMessageReady m_OnMessageReadyCallback;			// 接收数据时 callback
	OnTcpConnectionStart m_OnConnStart;					// 当连接建立 callback
	OnTcpConnectionEnd m_OnConnEnd;						// 当连接断开 callback
	void* m_UserCookie;
	ConnectionList m_ConnectionList;					// map (hash_key   TcpReassemblyData)
	ConnectionInfoList m_ConnectionInfo;				// 存储连接信息
	...
	time_t m_PurgeTimepoint;							// 完全清除时间点
