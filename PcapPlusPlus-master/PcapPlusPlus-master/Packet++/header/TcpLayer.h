...
#pragma pack(push,1)
	struct tcphdr {
		/** Source TCP port 网络字节序*/
		uint16_t portSrc;
		/** Destination TCP port 网络字节序*/
		uint16_t portDst;
		/** Sequence number */
		...
		uint16_t reserved:4,
		/** Specifies the size of the TCP header in 32-bit words 指出TCP头的长度*/
		dataOffset:4,
		/** FIN flag */
		finFlag:1,
		/** SYN flag */
...
#pragma pack(pop)
	...
	/**
	 * TCP options types  TCP选项类型,紧跟在TCP固定头部20字节长度后
	 */
	enum TcpOptionType {
		/** Padding 无操作，用于补位*/
		PCPP_TCPOPT_NOP =       1,
		/** End of options 选项列表结束*/
		PCPP_TCPOPT_EOL =       0,
		/** Segment size negotiating 最大Segment长度*/
		TCPOPT_MSS =          	2,
		/** Window scaling 窗口扩大系数*/
		PCPP_TCPOPT_WINDOW =    3,
		/** SACK Permitted 支持SACK*/
		TCPOPT_SACK_PERM =      4,
		/** SACK Block 收到SACK乱序*/
		PCPP_TCPOPT_SACK =      5,
...
#define PCPP_TCPOLEN_EXP_MIN        2
	...
	public:
		...
		// implement abstract methods   实现虚函数
	...
	public:
		 ...
		 * @param[in] packet A pointer to the Packet instance where layer will be stored in		Layer将保存在该Packet中
		 */
		...
		/**
		 * 计算 TCP head 和 data ，并选择是否将结果写入 tcphdr#headerChecksum
	...
	private:
		...
		TLVRecordReader<TcpOption> m_OptionReader;				// 选项读取器
		int m_NumOfTrailingBytes;								// 后续的byte数量

		void initLayer();
		uint8_t* getOptionsBasePtr() const { return m_Data + sizeof(tcphdr); }					// 获得 TCP选项的指针地址
