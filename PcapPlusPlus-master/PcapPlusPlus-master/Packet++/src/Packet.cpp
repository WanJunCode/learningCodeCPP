	...
	m_FreeRawPacket(true)
{
	...
	// Packet 与 RawPacket 是一对一的关系， 创建的内存由 m_RawPacket 进行释放
...
void Packet::setRawPacket(RawPacket* rawPacket, bool freeRawPacket, ProtocolType parseUntil, OsiModelLayer parseUntilLayer)
{
	...
	m_FreeRawPacket = freeRawPacket;								// 是否负责 未处理数据包的释放
	m_RawPacket = rawPacket;
	if (m_RawPacket == NULL)
		return;

	LinkLayerType linkType = m_RawPacket->getLinkLayerType();		// 获得链路层类型

	m_FirstLayer = createFirstLayer(linkType);						// 根据链路层类型创建第一层layer
...
void Packet::destructPacketData()
{
	...
	{
		delete m_RawPacket;			// 负责内部 未处理数据包的清理
	}
}
...
bool Packet::extendLayer(Layer* layer, int offsetInLayer, size_t numOfBytesToExtend)
{
	...
	m_RawPacket->insertData(indexToInsertData, tempData, numOfBytesToExtend);			// 内部使用 memcpy 进行复制
	delete[] tempData;

	// re-calculate all layers data ptr and data length 重新计算所有的layer 数据指针 和 数据长度
	const uint8_t* dataPtr = m_RawPacket->getRawData();

	// go over all layers from the first layer to the last layer and set the data ptr and data length for each layer
	Layer* curLayer = m_FirstLayer;
	bool passedExtendedLayer = false;			// 判断是否pass到了当前被扩展的layer
	while (curLayer != NULL)
	{
		// set the data ptr
		curLayer->m_Data = (uint8_t*)dataPtr;
		...
		// assuming header length of the layer that requested to be extended hasn't been enlarged(放大) yet
