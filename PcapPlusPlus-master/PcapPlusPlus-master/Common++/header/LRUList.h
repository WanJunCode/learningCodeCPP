/**
 * 	总结：底层使用 map 和 list 完成 LRU 的功能
 * 	list 用于存储元素， map 提供快速查找元素的能力
 **/
...
namespace pcpp
{
	...
	public:
		...
		int put(const T& element, T* deletedValue = NULL)
		{
			...
			// Inserting a new element. If an element with an equivalent key already exists the method returns an iterator to 
			...
			{
				m_CacheItemsList.erase(pair.first->second);			// 删除map中该迭代器之前对应的 second 值，修改为当前list中最前端值
				pair.first->second = m_CacheItemsList.begin();
			}

			if (m_CacheItemsMap.size() > m_MaxSize)
			{
				ListIterator lruIter = m_CacheItemsList.end();		// 指向map中最后元素的下一个位置
				lruIter--;
	...
	private:
		std::list<T> m_CacheItemsList;							// list 维护 LRU 队列
		std::map<T, ListIterator> m_CacheItemsMap;				// second 值指向 list 的迭代器
		size_t m_MaxSize;										// list 的最大值
	};
